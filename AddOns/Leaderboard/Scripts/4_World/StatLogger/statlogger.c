class StatLogger
{
	...
	void UpdateIngameStats(SurvivorBase sbPlayer) 
    { 		
		...
		statModelDisconnect.lastTimeSeen = Helper.GetCurrentTime();
		Save(statModelDisconnect, sbPlayer.GetPlayerID());
    }
	
	void ResetCurrentLifeStats(PlayerStatsModel statModel) 
    {
		statModel.longestShot = 0;
        statModel.zKilled = 0;
        statModel.timeSurvived = 0;
        statModel.distTrav = 0;
		statModel.lastTimeSeen = Helper.GetCurrentTime();
	}
    ...
    void DeathHandler(PlayerBase pbKilled, Object source) 
    {
		...
		ResetCurrentLifeStats(statModelDied);
